...
def get_time_extent(db):
    ...
    t0 = db['changelog'].find({}, {'time' : 1 , '_id' : 0}).sort([('time', pymongo.ASCENDING)]).limit(1)[0]['time']
    t1 = db['changelog'].find({}, {'time' : 1 , '_id' : 0}).sort([('time', pymongo.DESCENDING)]).limit(1)[0]['time']
   ...
   for i in range(bookmark, len(list_splits)):
      ...
      try:
         ...
         if (fieldorder_cmp(chunk['min'], split['min'], 'lt')):
          ...
          print('typeerror: split: ' +  dumps(split) + ', chunk: ' + dumps(chunk))
          print(e)

   if (iterated == True):
      print('Warning: ['+ str(bookmark) +'] split: ' + dumps(split), ' found to be ahead of chunk: ' + dumps(chunk['min']))
      return None, i
...
def build_split_list(db, ns, t0, t1):
    ...
    changelog_son = db['changelog'].with_options(codec_options=CodecOptions(document_class=SON))

    split_pattern = re.compile('split')
    splits_count = db['changelog'].count({'ns': ns, 'what': split_pattern, 'details.number' : {'$ne' : 1}, 'time': {'$gte': t0}})

    pipeline = [
       {'$match': {'ns': ns, 'what': split_pattern, 'details.number' : {'$ne' : 1}, 'time': {'$gte': t0}}},
    ...
    for split in changelog_son.aggregate(pipeline, allowDiskUse=True):
        ...
        if found:
           if(verbose):
              print('Found: ' + dumps(split['details']['before']) + ' included in '+ dumps(chunk))
        else:
...
def print_stats(db, ns, list_splits, t0, t1):
    ...
    splits_total = db.changelog.count({'what': split_pattern, 'ns': ns, 'details.number' : {'$ne' : 1}, 'time': {'$gte': t0}})
    ...
    print('   Chunks in the collection at ' + str(t1) + ': ' + str(chunks_total))
    print('   Chunks in the collection at ' + str(t0) + ' (plot): ' + str(len(final_list)))

def build_split_distribution(db, ns, no_timeout):

    print('Building split distribution...')

    chunks_son = db['chunks'].with_options(codec_options=CodecOptions(document_class=SON))
    i_splits = 0
    
    chunks_count = db['chunks'].count({'ns': ns})

    if (no_progressbar == False):
       pbar = ProgressBar(widgets=[Percentage(), Bar()], maxval=chunks_count).start()
       bar_i = 0

    bookmark = 0
    chunks_cursor = chunks_son.find({'ns': ns}, {'_id':0, 'min':1, 'max':1}, no_cursor_timeout=no_timeout).sort([('min', pymongo.ASCENDING)])
    while(1):
        try:
...
def get_starttime(t0, t1, args):
    ...
    if tstart < t0 or tstart > t1:
        raise Exception('starttime of ' + str(tstart) + ' is not within the time window [' + str(t0) + ', '+ str(t1) +']')
    else:
        t0 = tstart
...
def validate_actionlog(db, t0):
    ...
    for resharded in db['changelog'].find({'ns': ns, 'what': 'shardCollection.end', 'time': {'$gte': t0}}).sort([('time', pymongo.DESCENDING)]).limit(1):
        ...
        print(ns + ' was sharded on ' + str(resharded_time) + ', current starttime: ' + str(t0) + '. Please specify a higher --starttime.')
    ...
    for merged in db['changelog'].find({'ns': ns, 'what': 'merge', 'time': {'$gte': t0}}).sort([('time', pymongo.DESCENDING)]).limit(1):
        ...
        print(ns + ' underwent a chunk merge on ' + str(merged_time) + ', current starttime: ' + str(t0) + '. This tool does not support mergeChunk. Please specify a higher --starttime.')
        raise Exception('Time window cannot cover chunk merge')


if __name__ == '__main__':
    parser = argparse.ArgumentParser()
    parser.add_argument('-u', '--mongo_uri', type=str, help='connection string')
    parser.add_argument('-s', '--starttime', type=str, help='specify a start time (UTC 8601)')
    parser.add_argument('-v', '--verbose', help='print verbose information', action='store_true')
    parser.add_argument('-n', '--no_progressbar', help='disable progress bar', action='store_true')
    parser.add_argument('-N', '--no_timeout', help='use noCursorTimeout (advanced)', action='store_true')
    ...
    if db['collections'].count({'_id': ns, 'dropped': False}) == 0:
        raise Exception('Namespace ' + ns + ' does not exist or is not sharded')
